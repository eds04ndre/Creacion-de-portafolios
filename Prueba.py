...
with tab2:
    ...
    def calcular_metricas_portfolio():
        ...
        # Por ahora, generamos valores simulados para demostración
        
        metricas = {
            'rendimiento': np.random.uniform(5, 15),
            'volatilidad': np.random.uniform(10, 25),
            'beta': np.random.uniform(0.7, 1.3),
            'sharpe': np.random.uniform(0.5, 2.0),
            'sortino': np.random.uniform(0.6, 2.5),
            'max_drawdown': -np.random.uniform(10, 30),
            'var': -np.random.uniform(2, 5),
            'cvar': -np.random.uniform(3, 7),
            'sesgo': np.random.uniform(-0.5, 0.5),
            'curtosis': np.random.uniform(-1, 3),
            'rend_positivos': np.random.uniform(50, 70),
            'delta_benchmark': np.random.uniform(-2, 5)
        }
        return metricas
    ...
    if st.button("📊 Calcular Métricas", type="primary", use_container_width=True):
        with st.spinner("Calculando métricas..."):
            ...
            # Calcular métricas
            st.session_state.metricas = calcular_metricas_portfolio()
